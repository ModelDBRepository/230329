...
alphaQ=min(0.00002*Ca,0.01);betaQ=0.001;
wuqQ=alphaQ/(alphaQ+betaQ);tauQ=800;
...
%%
  ...
  dxi(2) = (wuqN-n)/tauN;
  dxi(3) = (wuqW-w)/tauW;
  dxi(4) = (1/C)*(Id-Ids/(1-p)-gca*a*a*h*(Vd-Vca)-gahp*q*(Vd-Vk)-gdl*(Vd-Vdl));
  dxi(5) = (wuqA-a)/tauA;
  dxi(6) = (wuqH-h)/tauH;
  dxi(7) = (wuqQ-q)/tauQ;
  dxi(8) = -0.13*(gca*a*a*h*(Vd-Vca))-0.075*Ca;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  y = [dxi];